...
export const sendContactEmails = async (data: ContactFormData) => {
  ...
  let caEmail = process.env.EMAIL_USER || 'gauravtcbd8@gmail.com';
  ...
  // 1. Try sending to target CA Email
  let leadResult = await resend.emails.send({
  ...
  // If Resend Sandbox restricts recipient, fallback automatically to Resend account owner email
  if (leadResult.error && leadResult.error.message?.includes('testing emails to your own email address')) {
    console.warn('Resend testing mode restriction: falling back to Resend account owner email (gauravgarg9595@gmail.com)');
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 24px; color: #1e293b; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
          ...
          <p style="font-size: 15px; color: #334155;">Dear <strong>${data.name}</strong>,</p>
          <p style="font-size: 15px; color: #334155;">Thank you for reaching out. We have received your inquiry and a member of our team will get back to you shortly.</p>
          <br/>
